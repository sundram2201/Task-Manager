      ...
      return (
        <>
          {localStorage?.role == "admin" ? (
            <Tag color={e == "progress" ? "blue" : e == "pending" ? "red" : "green"} key={e}>
              {e.toUpperCase()}
            </Tag>
          ) : (
            <Select placeholder='Select Status' defaultValue={e} onChange={(e) => updatedTask(e, i?._id)} allowClear>
...
const Tasks = () => {
  ...
  const onFinishFailed = (errorInfo) => {
    console.log("Failed:1111", errorInfo);
  };
    ...
    <>
      ...
      <Modal title='Create a Task' open={isModalOpen} onCancel={handleCancel}>
        <Form
          ...
          <Form.Item label='Remarks' name='remarks' rules={[{ required: true, message: "Please enter your Remarks!" }]}>
            <Input />
          </Form.Item>
